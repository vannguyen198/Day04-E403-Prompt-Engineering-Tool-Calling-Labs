...
class OrderDataStore:
    ...
    def __init__(self, data_dir: Path, output_dir: Path, *, today: str | None = None) -> None:
        ...
        # Initialize directories and load products.json
        self.output_dir.mkdir(parents=True, exist_ok=True)
        self.today = today or "2026-06-01"
        raw_products = __import__("json").loads((self.data_dir / "products.json").read_text(encoding="utf-8"))
    ...
    def list_products(
        ...
        *,
        ...
        """
        # Basic text normalization and search
        import unicodedata, re
    ...
    def get_product_details(self, product_ids: list[str]) -> list[dict]:
        ...
        # Build deterministic token based on found product ids
        found_product_ids = [item["product_id"] for item in details if item.get("status") == "ok"]
        token = self.build_detail_token(found_product_ids) if found_product_ids else ""
    ...
    def calculate_order_totals(self, *, items: list[OrderLineInput], detail_token: str, discount_rate: float) -> dict:
        ...
        if not self.validate_detail_token(requested_product_ids, detail_token):
            return {"status": "error", "errors": ["Invalid detail token. Call get_product_details again before pricing this order."]}
    ...
    def save_order(
        ...
        *,
        ...
        seed_payload = __import__("json").dumps(
            {
        ...
        payload = {
            ...
            "discount": {"campaign_code": campaign_code, "customer_tier": customer_tier},
            "notes": notes.strip(),
            "save_path": str(relative_path),
            "source": "llm-order-agent",
        }
        absolute_path.write_text(__import__("json").dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
